...
#asctime 当前时间  。 levelname   消息的等级info debug error。message 消息内容。
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        #
            ...
            with open(output_eval_file, "a+") as writer:
                writer.write('\n')
                writer.write("test %d:" % (i + 1))
                for key in sorted(test_result.keys()):
                    writer.write("%s = %s," % (key, str(test_result[key])))

                writer.write('\n')
                writer.write("valid %d:" % (i + 1))
                for key in sorted(valid_result.keys()):
                    writer.write("%s = %sf," % (key, str(valid_result[key])))

                writer.write('\n')
                writer.write("train %d:" % (i + 1))
                writer.write("acc = %.4f:" % (train_acc[i]))
