...
#asctime 当前时间  。 levelname   消息的等级info debug error。message 消息内容。
    ...
    if args.do_train:
        ...
        if output_mode == "classification":
            ...
            # 打印所有的训练集标签
            all_train_label = all_label_ids.data.cpu().numpy()
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for i in trange(int(args.num_train_epochs), desc="Epoch"):#_相当于i，计数器，但是在循环中用不着它
            tr_loss = 0
            ...
            for step, batch in enumerate(pbar):
                ...
                if len(train_preds) == 0:
                    train_preds.append(logits.view(-1, num_labels).data.cpu().numpy())
                else:
        ...
        #
            ...
            print("%d轮，train_acc:" %(i+1),end="")
            print(train_acc)
            print("%d轮，valid:"%(i+1),end="")
            print(valid_result)
            print("%d轮，test:"%(i+1),end="")
            print(test_result)
