...
import sys
# sys.path.append('E:/python/project/EAAI-25-master/src/model/pytorch_pretrained_bert/')
# sys.path.append('../pytorch_pretrained_bert/')
#modeling .BertForPreTraining tokenization.BertTokenizer optimization.BertAdam+ WarmupLinearSchedule
from pytorch_pretrained_bert import modeling#又见你了，朋友
...
def main():
    ...
    # Prepare model
    model = modeling.BertForPreTraining.from_pretrained(args.bert_model)
    if args.fp16:
        model.half()
    model.to(device)
    if args.local_rank != -1:
