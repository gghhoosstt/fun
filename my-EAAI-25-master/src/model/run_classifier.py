...
class SASProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            # print("reference answer:",text_a)
            # print("student answer:",text_b)
            if i <= 5:
...
def convert_examples_to_features(examples, label_list, max_seq_length,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        tokens = ["[CLS]"] + tokens_q + ["[SEP]"] + tokens_a + ["[SEP]"]
        ...
        input_mask += padding
        segment_ids += padding
        # print("input_id的长度",len(input_ids))
...
#asctime 当前时间  。 levelname   消息的等级info debug error。message 消息内容。
    logging.basicConfig(format = '%(asctime)s - %(levelname)s - %(name)s -  %(lineno)s -   %(message)s',
                        ...
                        filename = "F:\PYTHONproject\my-EAAI-25-master\log\mylog.txt"
                        )
    ...
    #加载traindata
    if args.do_train:
        train_features,both_answers = convert_examples_to_features(
        ...
        if output_mode == "classification":
            ...
            # 打印所有的训练集标签
            all_train_label = all_label_ids.data.numpy()
            epoch = int(args.num_train_epochs)
            # 复制epoch遍
            #all_train_label = np.tile(all_train_label,epoch)
            #print("训练集真实值：",all_train_label)
        elif output_mode == "regression":
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        eval_features,both_answers = convert_examples_to_features(
        ...
        if output_mode == "classification":
            ...
            all_test_label = all_label_ids.data.numpy()
        ...
        train_acc = []#训练集的学习曲线
        test_acc = []#训练集的学习曲线
        train_preds = []#训练集的预测值，len = epoch * 训练集数据条数
        preds = []#测试集的预测
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):#_相当于i，计数器，但是在循环中用不着它
            tr_loss = 0
            ...
            for step, batch in enumerate(pbar):
                ...
                if len(train_preds) == 0:
                    train_preds.append(logits.view(-1, num_labels).data.numpy())
                else:
                    train_preds[0] = np.append(train_preds[0], logits.view(-1, num_labels).data.numpy(), axis=0)
            ...
            # print("训练集的预测值,长度为",len(train_preds))
            train_acc = np.append(train_acc,accuracy_score(y_pred=train_preds, y_true=all_train_label))
            train_preds = []#本epoch清空，下轮重新计算acc
        ...
        #         output_model_file = os.path.join(args.output_dir, file_utils.WEIGHTS_NAME)
        #         output_config_file = os.path.join(args.output_dir,file_utils.CONFIG_NAME)
        #
        ...
        #
        #         model = modeling.BertForSequenceClassification.from_pretrained(args.output_dir, num_labels=num_labels)
        #         tokenizer = tokenization.BertTokenizer.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case)
        #     else:
        #         model = modeling.BertForSequenceClassification.from_pretrained(args.bert_model, num_labels=num_labels)
        #     model.to(device)
        # #做测试，testdata
        #     if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        #         eval_examples = processor.get_dev_examples(args.test_data_dir)
        ...
        #
            ...
            # 返回numpy数组中最大值的索引，即为分类结果
                ...
                # print("打印所有测试集的预测结果，长度为：",len(preds))
                # for i in range(len(preds)):
                #     print(preds[i], end=" ")
                # print()
            elif output_mode == "regression":
            ...
            result = compute_metrics(task_name, preds, all_label_ids.numpy(),both_answers)
            loss = tr_loss/global_step if args.do_train else None
            # all_test_label = all_label_ids.numpy();
            # print("测试集的真实值,长度为:",len(all_test_label))
            # for i in range(len(all_test_label)):
            #     print(all_test_label[i], end=" ")
            # print()
            test_acc = np.append(test_acc, accuracy_score(y_pred=preds, y_true=all_test_label))
            preds = []#本epoch清空预测值
            model.train()
        ...
        #epoch 外，画曲线
        paint_learning_curve(train_acc,test_acc,epoch)

        result['eval_loss'] = eval_loss #平均每条测试数据的损失
        result['global_step'] = global_step#完成所有epoch用的iteration数
        result['loss'] = loss #平均每条训练数据的损失
        # 写入文件
